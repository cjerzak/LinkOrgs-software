...
- **Use various matching techniques** (fuzzy string distance, ML embeddings, transfer learning, network-based matching, etc.) with the main `LinkOrgs()` function.
...
- **Network-based:** Markov or Bipartite-based matching using a LinkedIn-derived directory of aliases.
- **Network + ML:** A combination of ML embeddings and network-based approaches, accessed with `algorithm = "bipartite"` or `"markov"` and `DistanceMeasure = "ml"`.
- **Transfer:** A text-model transfer representation approach using the `text` package.
...
The network + ML approach merges the strengths of ML embeddings and the LinkedIn network. You can access it by setting:
...
In small tasks, the difference might be minor, but in larger tasks with trickier alias issues, the combined approach can improve recall at the same precision.
...
If you store large CSVs, ZIP files, or gzipped CSV files of data (possibly for replication) on Dropbox or another URL, the LinkOrgs package provides convenience functions for automatic downloading and importing into R:
...
- Internal helpers such as factor conversion and missing-value imputation are used by the package but are not exported as user-facing functions.
...
- **Network-based Approach:** Linking to the LinkedIn-derived directory can take time. If you have thousands of organizations, be prepared for a long-running process. Downloaded artifacts are cached; set `LINKORGS_CACHE_DIR` to choose the cache location.
